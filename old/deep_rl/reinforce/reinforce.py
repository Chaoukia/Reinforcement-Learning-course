...
class Reinforce:
    ...
    def unroll(self, env, train=True):
        ...
        reward_episode = 0
        while not done:
            action, action_logprob, entropy = self.action(state, train)
    ...
    def learn(self, states, actions_logprobs, rewards, entropies, alpha_entropy, optimizer):
        """
        Description
        -------------------
    ...
    def train(self, env, n_episodes=1000, lr=1e-4, alpha_entropy=0.1, thresh=250, file_save='dqn.pth', log_dir='runs/', print_iter=10):
        ...
        for episode in range(n_episodes):
            states, actions_logprobs, rewards, entropies, reward_episode = self.unroll(env)
    ...
    def test(self, env, n_episodes=1000, verbose=False):
        ...
        for episode in range(n_episodes):
            _, _, _, _, reward_episode = self.unroll(env, train=False)
