    ...
    def update_q_value(self, state, action, reward, next_state, alpha):
        ...
        try:
            if bern == 0:
                ...
                q_max = self.q_values_2[next_state][action_next]

            else:
                action_next = self.q_values_2[next_state].argmax()
                q_max = self.q_values_1[next_state][action_next]
        ...
        try:
            td = reward + self.gamma*q_max - self.q_values_1[state][action] if bern == 0 else reward + self.gamma*q_max - self.q_values_2[state][action]
